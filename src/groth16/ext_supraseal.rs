...
use crate::{
    constraint_system::{Circuit, SynthesisError},
    gpu,
    groth16::{
...
pub fn create_proof_batch_nonzk<E, C>(
    circuits: Vec<C>,
    params: &SuprasealParameters<E>,
) -> Result<Vec<Proof<E>>, SynthesisError>
where
    ...
    E::G2Affine: gpu::GpuName,
{
    create_proof_batch_priority_nonzk(circuits, params, false)
}
...
/// When several proofs are run in parallel on the GPU, it will get priority and will never be
/// aborted or pushed down to CPU.
pub fn create_proof_batch_nonzk_in_priority<E, C>(
    circuits: Vec<C>,
    params: &SuprasealParameters<E>,
) -> Result<Vec<Proof<E>>, SynthesisError>
where
    ...
    E::G2Affine: gpu::GpuName,
{
    create_proof_batch_priority_nonzk(circuits, params, true)
}
